...
class MarketMonitor:
    ...
    def _fetch_market_data(self, symbols: List[str]) -> Dict[str, Dict]:
        ...
        for symbol in symbols:
            try:
                ...
                recommendation = self.market_analyzer.analyze_trading_opportunity(
                    symbol
                )
                timing = self.market_analyzer.analyze_entry_timing(symbol)

                if all([ticker, market_metrics, recommendation, timing]):
                    current_price = float(ticker["lastPrice"])

                    if recommendation.stop_loss and recommendation.take_profit:
                        stop_loss_percent = (
                            (recommendation.stop_loss - current_price) / current_price
                        ) * 100
                        take_profit_percent = (
                            (recommendation.take_profit - current_price) / current_price
                        ) * 100
                    else:
                        stop_loss_percent = take_profit_percent = 0
                ...
                print(
                    ConsoleColors.error(
                        f"Error obteniendo datos para {symbol}: {str(e)}"
                    )
                )
        return market_data
    ...
    def _process_alerts(self, symbol: str, recommendation, timing, market_data: Dict):
        try:
            ...
            print(
                ConsoleColors.success(f"Análisis y alertas actualizados para {symbol}")
            )

        except Exception as e:
            print(
                ConsoleColors.error(f"Error procesando alertas para {symbol}: {str(e)}")
            )
