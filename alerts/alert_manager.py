...
class AlertManager:
    ...
    def _initialize_alerts_file(self):
        if not os.path.exists('alerts.json'):
            try:
                ...
                print(ConsoleColors.error(f"Error creando archivo de alertas: {str(e)}"))

    def add_price_alert(self, symbol: str, target_price: float, current_price: float,
                       condition: str = 'above', additional_info: Dict = None) -> None:
        message = f"{symbol} ha {'superado' if condition == 'above' else 'caído por debajo de'} {target_price:.8f}"
        self._add_alert('price', symbol, target_price, current_price, condition, message, additional_info)

    def add_support_resistance_alert(self, symbol: str, level: float, level_type: str,
                                   current_price: float, additional_info: Dict = None) -> None:
        message = f"{symbol} ha tocado el nivel de {level_type} en {level:.8f}"
        self._add_alert(level_type, symbol, level, current_price, 'touch', message, additional_info)

    def _add_alert(self, alert_type: str, symbol: str, target_price: float,
                  current_price: float, condition: str, message: str, additional_info: Dict = None) -> None:
        alert = Alert(
            symbol=symbol,
            type=alert_type,
            condition=condition,
    ...
    def check_alerts(self, market_data: Dict[str, Dict]) -> None:
        for alert in self.alerts:
            ...
            current_price = market_data.get(alert.symbol, {}).get('price', 0)
            signal = market_data.get(alert.symbol, {}).get('signal')
            strength = market_data.get(alert.symbol, {}).get('strength')
            entry_price = market_data.get(alert.symbol, {}).get('entry_price', 0)
            stop_loss = market_data.get(alert.symbol, {}).get('stop_loss', 0)
            take_profit = market_data.get(alert.symbol, {}).get('take_profit', 0)
            stop_loss_percent = market_data.get(alert.symbol, {}).get('stop_loss_percent', 0)
            take_profit_percent = market_data.get(alert.symbol, {}).get('take_profit_percent', 0)
            ...
            if self._should_trigger_alert(alert, current_price):
                self._trigger_alert(alert, signal, strength, entry_price, stop_loss, take_profit,  stop_loss_percent, take_profit_percent)

    def _should_trigger_alert(self, alert: Alert, current_price: float) -> bool:
        if alert.type == 'price':
            return (current_price >= alert.target_price if alert.condition == 'above'
    ...
    def _trigger_alert(self, alert: Alert, signal, strength, entry_price, stop_loss, take_profit, stop_loss_percent, take_profit_percent) -> None:
        try:
            message = self._generate_alert_message(alert, signal, strength, entry_price, stop_loss, take_profit, stop_loss_percent, take_profit_percent)
    ...
    def _generate_alert_message(self, alert: Alert, signal: str, strength: str, entry_price: float, stop_loss: float, take_profit: float, stop_loss_percent: float, take_profit_percent: float) -> str:
        try:
...
"""
            ...
            if all(k in info for k in ['target_price', 'stop_loss', 'take_profit']):
                stop_loss_percent = ((info['stop_loss'] - alert.current_price) / alert.current_price * 100)
                take_profit_percent = ((info['take_profit'] - alert.current_price) / alert.current_price * 100)
                message += f"""
...
"""
    ...
    def load_alerts(self) -> None:
        try:
            ...
            print(ConsoleColors.warning("Archivo de alertas vacío o corrupto. Creando nuevo archivo."))
            self._initialize_alerts_file()
        except Exception as e:
