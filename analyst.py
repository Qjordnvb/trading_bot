...
class MemeCoinAnalyzer:
    def __init__(self, cmc_api_key: str):
        ...
        self.meme_keywords = {
            ...
            'secondary': ['elon', 'wojak', 'chad', 'cat', 'kitty', 'safe', 'moon', 'rocket',
                         'baby', 'pup', 'pug', 'shiba', 'akita', 'corgi', 'moon'],
            'exclude': ['chain', 'swap', 'protocol', 'finance']  # Palabras para excluir falsos positivos
        }
    ...
    def get_top_meme_coins(self) -> List[Dict]:
        try:
            ...
            try:
                ...
                market_sentiment = self.market_analyzer.calculate_market_sentiment(market_metrics)
            except Exception as e:
            ...
            for coin in meme_coins:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        trading_signal, signal_reasons = self.analyze_trading_signal(coin_info, market_sentiment)
                        ...
                        if attempt == max_retries - 1:
                            print(f"Error analizando {coin.get('symbol', 'Unknown')}: {str(e)}")
                        else:
                            time.sleep(1)
                        continue
    ...
    def _is_meme_coin(self, coin: Dict) -> bool:
        ...
        if any(word in name_lower or word in symbol_lower for word in self.meme_keywords['exclude']):
            return False

        # Verificar keywords primarios (mayor peso)
        primary_match = any(keyword in name_lower or keyword in symbol_lower
                          for keyword in self.meme_keywords['primary'])
        if primary_match:
            return True

        # Verificar keywords secundarios (requiere múltiples coincidencias)
        secondary_matches = sum(1 for keyword in self.meme_keywords['secondary']
    ...
    def _meets_basic_criteria(self, coin: Dict) -> bool:
        ...
        try:
            ...
            return (
                market_cap >= self.thresholds['min_market_cap'] and
                volume_24h >= self.thresholds['min_volume'] and
                (volume_24h / market_cap if market_cap > 0 else 0) >= self.thresholds['min_liquidity_ratio']
            )
        except Exception:
            return False
    ...
    def analyze_metrics(self, coin_data: Dict) -> Tuple[float, List[str]]:
        ...
        try:
            ...
            if self.thresholds['ideal_market_cap']['min'] <= market_cap <= self.thresholds['ideal_market_cap']['max']:
                score += 3
    ...
    def _analyze_liquidity(self, quote: Dict) -> Tuple[float, List[str]]:
        ...
        if market_cap and market_cap > 0:
            ...
            if volume_to_mcap >= 0.5:
                score += 5
                reasons.append(f"Liquidez excepcional (V/MC: {volume_to_mcap*100:.1f}%)")
    ...
    def analyze_trading_signal(self, coin_data: Dict, market_sentiment: float) -> Tuple[TradingSignal, List[str]]:
    ...
    def _analyze_volume_signal(self, volume_24h: float, market_cap: float) -> Tuple[float, List[str]]:
    ...
    def _analyze_momentum_signal(self, change_24h: float, change_7d: float) -> Tuple[float, List[str]]:
    ...
    def _analyze_overbought(self, change_24h: float, change_7d: float) -> Tuple[float, List[str]]:
    ...
    def _get_coin_slug(self, coin_id: int) -> Dict:
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error obteniendo información de la moneda: {response.status_code}")
                return None
    ...
    def _validate_exchange_data(self, data: Dict) -> bool:
        ...
        return (
            ...
            'marketPairs' in data['data'] and
            isinstance(data['data']['marketPairs'], list) and
            len(data['data']['marketPairs']) > 0
        )
...
class MarketAnalyzer:
    ...
    def calculate_market_sentiment(self, metrics: Dict) -> float:
        ...
        try:
            ...
            # Métricas básicas
            market_cap_change = usd_data.get('total_market_cap_yesterday_percentage_change', 0)
            volume_change = usd_data.get('total_volume_24h_yesterday_percentage_change', 0)
            ...
            volatility_score = self._calculate_volatility_score(market_cap_change, volume_change)

            # Cálculo de liquidez
            liquidity_score = self._calculate_liquidity_score(volume_24h, market_cap)

            # Análisis de dominancia
            dominance_score = self._analyze_market_dominance(btc_dominance, altcoin_dominance)
    ...
    def _calculate_volatility_score(self, market_cap_change: float, volume_change: float) -> float:
    ...
    def _analyze_market_dominance(self, btc_dominance: float, altcoin_dominance: float) -> float:
...
class TopCryptoAnalyzer:
    ...
    def analyze_trading_signal(self, crypto_data: Dict, market_sentiment: float) -> TradingSignal:
    ...
    def _analyze_volume(self, usd_data: Dict) -> Dict:
        ...
        try:
            ...
            if volume_change_24h > self.thresholds['volume']['significant_increase']:
                score += 1
                reasons.append(f"Aumento significativo de volumen (+{volume_change_24h:.1f}%)")
            elif volume_change_24h < self.thresholds['volume']['significant_decrease']:
                score -= 1
                reasons.append(f"Disminución significativa de volumen ({volume_change_24h:.1f}%)")
...
class PromissingProjectsAnalyzer:
    ...
    def get_promising_projects(self) -> List[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    print(f"Reintentando obtener proyectos prometedores (intento {attempt + 1}/{max_retries})...")
                        ...
                        for project in response_data['data']:
                            try:
                                ...
                                if self._is_promising_project(project):
                                    ...
                                    if score > 0:
                                        trading_signal, signal_reasons = self.analyze_trading_signal(project, market_sentiment)
                                        analyzed_projects.append({
                                            'project': project,
                                            'score': score,
                                            'analysis': self._get_project_analysis(project),
                                            'signal': trading_signal.value,
                                            'signal_reasons': signal_reasons
                                        })
                                ...
                                print(f"Error al analizar proyecto {project.get('symbol', 'Unknown')}: {str(e)}")
                                continue

                        sorted_projects = sorted(analyzed_projects, key=lambda x: x['score'], reverse=True)[:10]
                        print(f"Análisis completado. Se encontraron {len(sorted_projects)} proyectos prometedores")
                        return sorted_projects

                    elif response_data['status'].get('error_code') == 1008:  # Rate limit exceeded
                        if attempt < max_retries - 1:
                            print("Límite de tasa excedido. Esperando para reintentar...")
                            continue
                        else:
                            print("Se alcanzó el límite de intentos por límite de tasa")
                            return []
                    else:
                        print(f"Error en la API: {response_data['status'].get('error_message')}")
                        return []
    ...
    def _is_promising_project(self, project: Dict) -> bool:
        ...
        try:
            ...
            return (
                market_cap > 10000000 and         # Market cap > $10M
                volume_24h > 1000000 and          # Volumen 24h > $1M
                market_cap < 10000000000 and      # Market cap < $10B
                volume_24h/market_cap > 0.01      # Ratio volumen/market cap > 1%
            )
        except Exception:
            return False
    ...
    def _calculate_project_score(self, project: Dict) -> float:
        ...
        volume_mcap_ratio = usd_data['volume_24h'] / usd_data['market_cap'] if usd_data['market_cap'] > 0 else 0
        liquidity_score = min(volume_mcap_ratio * 10, 5)

        # Factor de madurez del mercado
        market_maturity = min(project['circulating_supply'] / project['max_supply'] if project['max_supply'] else 0.5, 1)
    ...
    def _analyze_risks(self, project: Dict, usd_data: Dict) -> List[str]:
        ...
        if project['circulating_supply'] / project['max_supply'] if project['max_supply'] else 0 < 0.2:
    ...
    def _generate_recommendations(self, project: Dict, usd_data: Dict) -> List[str]:
        ...
        if usd_data['market_cap'] < 100000000:
            recommendations.append("Considerar como inversión de alto riesgo/alta recompensa")
    ...
    def analyze_trading_signal(self, project: Dict, market_sentiment: float) -> Tuple[TradingSignal, List[str]]:
        ...
        if technical_trend > 10:
            signal_score += 2
            signal_reasons.append(f"Tendencia técnica positiva ({technical_trend:.1f}%)")
        elif technical_trend < -10:
            signal_score -= 2
            signal_reasons.append(f"Tendencia técnica negativa ({technical_trend:.1f}%)")
        ...
        try:
            ...
            if top_cryptos:
                ...
                for crypto in top_cryptos:
                    signal = self.top_crypto_analyzer.analyze_trading_signal(crypto, result.get('market_sentiment', 0))
                    top_crypto_analysis.append({
                        'crypto': crypto,
                        'signal': signal
                    })
                result['top_cryptos'] = top_crypto_analysis
                print(f"Análisis completado para {len(top_crypto_analysis)} criptomonedas")
            ...
            if promising_projects:
                ...
                print(f"Análisis completado para {len(promising_projects)} proyectos prometedores")
...
def print_complete_analysis(analysis: Dict):
    ...
    if 'top_cryptos' in analysis:
        ...
        for crypto_analysis in analysis['top_cryptos']:
            ...
            print(f"Señal: {crypto_analysis['signal'].value}")  # Accedemos al value del enum
        ...
        for coin in analysis['meme_coins']:
            ...
            if coin.get('exchanges'):
                for exchange in coin['exchanges']:
                    print(f"  • {exchange.get('name', 'N/A')} ({exchange.get('exchange_slug', 'N/A')})")
...
def main():
    try:
        ...
        print(f"Usando API key: {API_KEY[:5]}..." if API_KEY else "API key no encontrada")
